...
public class TestApp {
    ...
    private final static int BUTTONS_PER_COLUMN = 10;
    private final static int MAX_VALUE = 1000;
    private final static int MORE_BUTTONS_VALUE = 30;

    private final static String INVALID_NUMBER_MSG = "Please enter a valid integer number";
    private final static String MORE_BUTTONS_MSG = "Please select a value smaller or equal to " + MORE_BUTTONS_VALUE;
    ...
    private void showSortScreen() {
        ...
        if (!sortScreenInitialized) {
            clearButtons();
            numbers = initRandomNumbers(numberForSortScreen);
            createButtons(numberForSortScreen);
    ...
    private JButton createNewButton(int index, JPanel columnPanel) {
        ...
        button.addActionListener(e -> {
            ...
            if (count <= MORE_BUTTONS_VALUE) {
                clearButtons();
                numbers.addAll(initRandomNumbers(count));
                createButtons(numbers.size());
                sortScreenInitialized = true;
            } else {
    ...
    private void showMessageError(Exception ex) {
        SwingUtilities.invokeLater(() ->
                JOptionPane.showMessageDialog(
                        frame,
                        "Сталася помилка під час оновлення кнопок: " + ex.getMessage(),
                        "Помилка",
    ...
    private static class IterativeQuickSort {
